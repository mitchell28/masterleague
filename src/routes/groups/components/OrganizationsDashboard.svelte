		...
		<div class="border-b-4 border-indigo-600 bg-slate-800/50 p-6">
			...
			{#if $hasActiveSubscription}
				...
				<button
					...
					class=" border border-red-600 bg-red-600/10 px-4 py-2 text-sm font-medium text-red-400 transition-colors hover:bg-red-600/20"
				>
					Manage Subscription
				</button>
			{:else}
				...
				<button
					...
					class=" bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-indigo-700"
				>
...
{#if showCreateOrganization}
	...
	>
		<div class="w-full max-w-md border border-slate-700 bg-slate-800 p-6">
			...
			<div class="space-y-4">
				<div>
					...
					<input
						...
						class="w-full border border-slate-600 bg-slate-700 px-3 py-2 text-white placeholder-slate-400"
					/>
				</div>
		...
		<div class="w-full max-w-md border border-slate-700 bg-slate-800 p-6">
			...
			<div class="mt-6 flex gap-3">
				<button
					...
					class="bg-accent flex-1 px-4 py-2 text-sm font-medium text-black transition-colors hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
				>
					Join
				</button>
				<button
					onclick={() => (showJoinOrganization = false)}
					class="flex-1 border border-slate-600 bg-slate-700 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-slate-600"
				>
					Cancel
				</button>
			</div>
...
{#if showSubscriptionPlans}
	<div
		...
		aria-label="Close modal"
	>
		<div class="w-full max-w-4xl border border-slate-700 bg-slate-800 p-6">
			...
			<div class="grid gap-6 md:grid-cols-3">
				{#each $availablePlans as plan}
					<div
						class="relative border p-6 transition-colors {plan.popular
					...
					>
						...
						<button
							...
							class="mt-6 w-full px-4 py-2 text-sm font-medium transition-colors {plan.popular
...
{#if toasts.length > 0}
	<div class="fixed top-4 right-4 z-[60] space-y-2">
		{#each toasts as toast (toast.id)}
			<div
				class="flex items-center justify-between border px-4 py-3 shadow-lg {toast.type ===
				'success'
