...
export class CacheTestSuite {
	...
	async testCachePerformance() {
		...
		for (const size of dataSizes) {
			...
			try {
				...
				this.recordResult(
					`Performance Write (${size} items)`,
					true,
					`${((size / writeTime) * 1000).toFixed(0)} items/sec`,
					writeTime
				);
			} catch (error) {
				this.recordResult(`Performance Write (${size} items)`, false, `Failed: ${error.message}`);
				continue;
			}
			...
			try {
				...
				if (result && result.data.length === size) {
					this.recordResult(
						`Performance Read (${size} items)`,
						true,
						`${((size / readTime) * 1000).toFixed(0)} items/sec`,
						readTime
					);
				} else {
					this.recordResult(`Performance Read (${size} items)`, false, 'Data size mismatch');
				}
			} catch (error) {
				this.recordResult(`Performance Read (${size} items)`, false, `Failed: ${error.message}`);
			}
	...
	async testConcurrentOperations() {
		...
		try {
			...
			this.recordResult(
				'Concurrent Writes',
				true,
				`${testSessions} concurrent writes completed`,
				duration
			);
		} catch (error) {
		...
		const readPromises = [];
		for (let i = 0; i < testSessions; i++) {
			readPromises.push(LeaderboardCache.get(`${this.organizationId}-test-${i}`, this.season));
		}
		...
		try {
			...
			this.recordResult(
				'Concurrent Reads',
				true,
				`${successCount}/${testSessions} concurrent reads successful`,
				duration
			);
		} catch (error) {
	...
	async testErrorHandling() {
		...
		try {
			...
			this.recordResult(
				'Error Handling - Large Data',
				true,
				`Handled large dataset (10k items)`,
				duration
			);
		} catch (error) {
			this.recordResult(
	...
	async testCacheInvalidation() {
		...
		try {
			...
			if (!result || result.data.length === 0) {
				...
				this.recordResult(
					'Cleanup - Verify Removal',
					false,
					'Data still present after invalidation'
				);
			}
		} catch (error) {
			this.recordResult(
				'Cleanup - Verify Removal',
				true,
				'Cache miss after invalidation (expected)'
			);
		}
	}
	...
	showCacheResults() {
		...
		if (performanceTests.length > 0) {
			const avgTiming =
				performanceTests.reduce((sum, r) => sum + r.timing, 0) / performanceTests.length;
